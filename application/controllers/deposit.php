...
//
            ...
            $data = array(
                'content' => $this->load->view('deposit/edit', $data, true),
            );
            $this->load->view('main_layout', $data);
        } else {
             $data = array(
                'content' => $this->load->view('deposit/edit', '', true),
            );
            $this->load->view('main_layout', $data);
        }
    }
    ...
    public function update() {
        ...
        if ($this->input->post('btn_submit') == 'บันทึก' || $this->input->post('btn_submit') == 'บันทึกและแก้ไขต่อ') {
            ...
            if ($this->db->update('deposit', $dataInsert)) {

                $this->session->set_flashdata('message_success', 'แก้ไขข้อมูลแล้ว');
               

                    redirect('deposit/edit/' . $this->input->post('id'));
