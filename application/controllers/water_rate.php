...
class water_rate extends CI_Controller {
...
//
            ...
            $data = array(
                'content' => $this->load->view('water_rate/edit', $data, true),
            );
            $this->load->view('main_layout', $data);
        } else {
             $data = array(
                'content' => $this->load->view('water_rate/edit', '', true),
            );
            $this->load->view('main_layout', $data);
        }
    }
    ...
    public function update() {
        ...
        if ($this->input->post('btn_submit') == 'บันทึก' || $this->input->post('btn_submit') == 'บันทึกและแก้ไขต่อ') {
            ...
            if ($this->db->update('water_rate', $dataInsert)) {

                $this->session->set_flashdata('message_success', 'แก้ไขข้อมูลแล้ว');
               

                    redirect('water_rate/edit/' . $this->input->post('id'));
